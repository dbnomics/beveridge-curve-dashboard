...
def load_unemployment_series():
    ...
    unemployment_series_fr = fetch_series("INSEE/CHOMAGE-TRIM-NATIONAL/T.CTTXC.TAUX.FM.0.00-.POURCENT.CVS.FALSE")
    unemployment_series_eu = fetch_series("OECD/MEI/EA20.LRHUTTTT.STSA.Q")
    return unemployment_series_ger, unemployment_series_us, unemployment_series_fr, unemployment_series_eu
...
def prepare_data(start_date=None, end_date=None):
    # Load data
    unemployment_series_ger, unemployment_series_us, unemployment_series_fr, unemployment_series_eu = load_unemployment_series()
    job_vacancy_ger, job_vacancy_us, job_vacancy_fr, job_vacancy_eu = load_job_vacancy_series()
    ...
    unemployment_series_fr = unemployment_series_fr[col_un_fr].rename(columns={"value": "unemployment rate"})
    df_fr = pd.merge(job_vacancy_fr, unemployment_series_fr, on="original_period", how="left").dropna()
    ...
    unemployment_series_us = unemployment_series_us[col_un_us].rename(columns={"value": "unemployment rate"})
    unemployment_series_us["Country"] = "United States"
    df_us = pd.merge(job_vacancy_us, unemployment_series_us, on="original_period", how="left").dropna()
    ...
    job_vacancy_ger = job_vacancy_ger[col_job_ger].rename(columns={"value": "job vacancy"})
    unemployment_series_ger = unemployment_series_ger[col_un_ger].rename(columns={"value": "unemployment rate"})
    df_ger = pd.merge(job_vacancy_ger, unemployment_series_ger, on="original_period", how="left").dropna()
    ...
    unemployment_series_eu = unemployment_series_eu[col_un_eu].rename(columns={"value": "unemployment rate"})
    df_eu = pd.merge(job_vacancy_eu, unemployment_series_eu, on="original_period", how="left").dropna()
